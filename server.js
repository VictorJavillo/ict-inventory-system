...
app.get("/signatories.html", sendProtectedPage("signatories.html", ["admin"]));
...
await pool.query(`
ALTER TABLE signatories
ADD COLUMN IF NOT EXISTS role TEXT DEFAULT 'Prepared By'
`);
...
/* GET ALL SIGNATORIES */
app.get("/api/signatories", requireLogin, async (req, res) => {
  try {
    ...
    console.error("Load signatories error:", error);
...
app.get("/api/test-storage", requireRole("admin"), async (req, res) => {
...
/* ADD SIGNATORY */
app.post("/api/signatories", requireRole("admin"), async (req, res) => {
    try {
...
app.put("/api/signatories/:id", requireRole("admin"), async (req, res) => {
...
app.delete("/api/signatories/:id", requireRole("admin"), async (req, res) => {
    ...
    try {
        ...
        const existing = await pool.query(
            `SELECT * FROM signatories WHERE id = $1`,
            [id]
        );
        ...
        await pool.query(
            `DELETE FROM signatories WHERE id = $1`,
            [id]
        );
        ...
        await logActivity(
            ...
            "DELETE",
            "SIGNATORIES",
            `Deleted signatory: ${existing.rows[0].fullname}`
        );
        ...
        console.error("Delete Signatory:", err);
